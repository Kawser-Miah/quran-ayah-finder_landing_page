...
let _cachedApp = null; // used to re-render QR codes on theme change
...
function applyTheme(theme) {
  ...
  if (_cachedApp) renderQRCodes(_cachedApp);
...
function renderQRCodes(app) {
  ...
  stores.forEach(({ url, label, icon }) => {
    ...
    // Always use high-contrast colors — the box has a fixed white CSS background
    // so the QR remains scannable in both light and dark mode
    new QRCode(box, {
      text: url,
      width: 128,
      height: 128,
